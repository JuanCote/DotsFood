...
class AddAddressCallbackTest extends TestCase
{
    /**
     * A basic test example.
     */
    public function testAddAddressCallbackHandle(): void
    {
        $usersServiceMock = Mockery::mock(UsersService::class);

        $addAddressCallback = new AddAddressCallback($usersServiceMock);
        ...
        $addAddressSenderMock = Mockery::mock(AddAddressSender::class);
        $addAddressSenderMock->shouldReceive('handle')->once()->andReturn();
        app()->instance(AddAddressSender::class, $addAddressSenderMock);

        $addAddressCallback->handle($callbackQuery);
    }
}
